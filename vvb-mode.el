...
;; Created:    Jan-25-1999
;; Version:    0.42-gnu-emacs
;; Keywords:   minor-mode, visual highlighting

;; Modified 2002-09-01 to work with GNU Emacs.  lawrence mitchell <wence@gmx.li>
...
;;      - added `vvb-map' to allow special key bindings for the visible
;;        vertical bar
...
;;      - fixed a bug in `vvb-set-column' thanks to Vilvio Picano
;;        <spicano@ptdcs2.intel.com>
;;      - minor enhancements to menubar
;; 0.41 05/09/2002
;;      - minor fix on attaching this mode to minor mode list to avoid
;;        duplicates thanks to Martin Kähmer
;;        <martink@demag.rwth-aachen.de>

;; 0.42 09/05/2002
;;      - added GNU Emacs support in the form of overlays.  From
;;        lawrence mitchell <wence@gmx.li>

;;; Code:
(require 'cl)
(eval-when-compile
  ;; shut up the bytecompiler in GNU Emacs.
  (defvar modeline-mousable-minor-mode-extent)
  (defconst vvb-xemacs-p (featurep 'xemacs)
  "Non-nil if this Emacs is in fact XEmacs."))
(eval-and-compile
  (autoload 'easy-menu-add-item "easymenu"))

;; GNU Emacs glue

(defmacro vvb-make-extent (b e)
  "Make an extent between B and E in XEmacs, and an overlay in Emacs."
  (if vvb-xemacs-p
      `(make-extent ,b ,e)
    `(make-overlay ,b ,e)))

(defmacro vvb-delete-extent (extent)
  "Delete EXTENT in XEmacs, or delete the overlay named EXTENT in Emacs."
  (if vvb-xemacs-p
      `(delete-extent ,extent)
    `(delete-overlay ,extent)))

(defmacro vvb-set-extent-property (extent prop value)
  "Set one property of extent EXTENT in XEmacs.

Set one property of overlay EXTENT in Emacs.

Give property PROP a value of VALUE."
  (if vvb-xemacs-p
      `(set-extent-property ,extent ,prop ,value)
    `(overlay-put ,extent ,prop ,value)))

(defmacro vvb-extent-at (pos dummy prop)
  "Non nil if there is an extent/overlay at POS with property PROP.

DUMMY is always nil, this is to keep the same argument order as XEmacs
`extent-at'."
  (if vvb-xemacs-p
      `(extent-at ,pos nil ,prop)
    `(memq ,prop (overlays-at ,pos))))

(defmacro vvb-add-submenu (path menu)
  "Add MENU as a submenu to the map with path PATH."
  (if vvb-xemacs-p
      `(add-submenu ,path ,menu)
    ;; Emacs uses downcased menu "path names" for it's menus
    `(easy-menu-add-item nil (list (downcase (car ,path))) ,menu)))

(defmacro vvb-redraw-modeline ()
  "Call `redraw-modeline' in XEmacs, `force-mode-line-update' in Emacs."
  (if vvb-xemacs-p
      `,(redraw-modeline)
    `,(force-mode-line-update)))
;;; end GNU Emacs glue.

(defconst vvb-version "v0.42-gnu-emacs 2002-09-01")
...
meaning that the column is visible only when `point' is over it.  Only
effective when `vvb-sticky-p' is nil.")
...
(define-key vvb-map (if vvb-xemacs-p 'button-3 [(mouse-3)]) 'vvb-popup-menu)
...
(defvar vvb-menu
  '("Vertical Bar"
    ...
    "--"
    ["Horizontal Line" (setq vvb-horizontal-p (null vvb-horizontal-p))
     :active vvb-mode :style toggle :selected vvb-horizontal-p]
    ["Sticky Bar" (setq vvb-sticky-p (null vvb-sticky-p))
     :active vvb-mode :style toggle :selected vvb-sticky-p]
    ["Permanent Bar" (setq vvb-permanent-p (null vvb-permanent-p))
     :active (and vvb-mode (null vvb-sticky-p))
     :style toggle :selected vvb-permanent-p]
    ["Extend to EOL" (setq vvb-right-on-eol-p (null vvb-right-on-eol-p))
     :active (and vvb-mode (null vvb-sticky-p))
     :style toggle :selected vvb-right-on-eol-p]
    "--"
    ["Toggle Multiple Columns" vvb-toggle-multi-columns
     :active vvb-mode :style toggle :selected vvb-columns]
    ...
    (save-excursion
      (if vvb-horizontal-p
          (let (b e x)
            ...
            (end-of-line)
            (setq e (point))
            (setq x (vvb-make-extent b e))
            (push x vvb-internal-cache)
            (vvb-set-extent-property x 'face vvb-face)
            (vvb-set-extent-property x 'keymap vvb-map)
            (vvb-set-extent-property x 'vvb t)))
...
(if vvb-xemacs-p
    (or (assq 'vvb-mode minor-mode-alist)
        (add-to-list 'minor-mode-alist
                     (list 'vvb-mode
                           (cons modeline-mousable-minor-mode-extent " +"))))
  (or (assq 'vvb-mode minor-mode-alist)
      (add-to-list 'minor-mode-alist
                   ;; Not sure what the variable
                   ;; `modeline-mousable-minor-mode-extent' actually
                   ;; does.  lawrence mitchell 2002-09-01
                   (list 'vvb-mode " Vbar"))))

(add-hook 'post-command-hook 'vvb-post-command-cb)

(global-set-key "\C-xc" 'vvb-set-column)
(vvb-add-submenu '("Tools") vvb-menu)
